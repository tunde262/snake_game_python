...
class Collision:
    ...
    def between_snake_and_apple(self, snake, apple):
        # Formula: Distance between two points (below)
...
async def main():
    ...
    while True:
        ...
        if collison.between_snake_and_apple(snake, apple):
            apple.spawn()
        ...
        if collison.between_snake_and_walls(snake):
            # lose
            snake.die()
            apple.spawn()
            score.reset()

        if collison.between_head_and_body(snake):
            # lose
            snake.die()
            apple.spawn()
            score.reset()
